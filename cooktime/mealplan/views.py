...
def search_recipes(request):
    ...
    if r.status_code == 200:
        ...
        return render(request, 'mealplan/search-recipes.html', context)


class RecipeBoxListView(LoginRequiredMixin, ListView):
    model = RecipeBox
    template_name = 'mealplan/recipe-box.html'
...
class ShoppingListView(LoginRequiredMixin, ListView):
    model = ShoppingList
    template_name = 'mealplan/shopping-lists.html'
...
class ShoppingDetailView(DetailView):
    model = ShoppingList
    template_name = 'mealplan/shoppinglists_detail.html'
...
class MealPlanCalendarView(LoginRequiredMixin, ListView):
    ...
    template_name = 'mealplan/meal-plan.html'
...
class MealPlanCreateView(LoginRequiredMixin, CreateView):
    template_name = 'mealplan/meal_plan_add.html'
    ...
    def get_form_kwargs(self):
        ...
        if kwargs.get('data'):
            ...
            kwargs.update(
                {'redirect': f"/meal-plan/{first_date.year}/{first_date.month}/{first_date.day}/"})
        else:
            path_info = (self.request.path).split('/meal-plan/')[1].split('/')
...
def aboutus(request):
    return render(request, 'mealplan/about-us.html')
...
def contact(request):
    ...
    form = ContactForm()
    return render(request, 'mealplan/contact.html', {'form': form})


def privacypolicy(request):
    return render(request, 'mealplan/privacy-policy.html')
...
def register(request):
    if request.method == 'POST':
        ...
        form = UserRegisterForm()
    return render(request, 'mealplan/register.html', {'form': form})
        ...
        if url:
            ...
            try:
                ...
                try:
                    ...
                    return render(request, 'mealplan/add-recipes-links.html', context)
                except IntegrityError:
                    messages.error(request, f"This recipe is already in your recipe box.")
